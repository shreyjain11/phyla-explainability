#CODE TAKEN DIRECTLY FROM https://github.com/niemasd/TreeCluster/blob/master/TreeCluster.py
#!/usr/bin/env python3
...
def argmax_clusters(method,tree,threshold,support):
    ...
    for i,t in enumerate(thresholds):
        ...
        num_non_singleton = len([c for c in clusters if len(c) > 5]) # TODO: Change minimum cluster size
...
# Helper function to run TreeCluster in script
def run_treecluster(tree_str, threshold):
    """
    Returns cluster information in dictionary
    """
    threshold_free = "argmax_clusters"
    method = "max_clade"
    support = float('-inf')
    # Convert tree string into tree
    tree = read_tree_newick(tree_str.strip())

    # Generate clusters
    clusters = THRESHOLDFREE[threshold_free](METHODS[method],tree,threshold,support)
    return clusters
